...
const COMPONENT_TEMPLATES: ComponentTemplate[] = [
  {
    ...
    preview: ({ config }: { config: ComponentConfig }) => (
...
export const useFormBuilder = () => {
  ...
  const updateComponent = useCallback((id: string, updates: Partial<ComponentConfig>) => {
    setLayoutData((prev: LayoutData) => ({
      ...prev,
      components: prev.components.map((comp: ComponentConfig) =>
  ...
  const deleteComponent = useCallback((id: string) => {
    setLayoutData((prev: LayoutData) => ({
      ...prev,
      components: prev.components.filter((comp: ComponentConfig) => comp.id !== id)
    }));
    ...
    setEditorState((prev: EditorState) => ({
      ...
      selection: {
        ...
        selectedComponents: prev.selection.selectedComponents.filter((compId: string) => compId !== id)
      }
    }));
  }, []);
        ...
        return {
          ...
          selection: {
            ...
            selectedComponents: isSelected
              ? prev.selection.selectedComponents.filter((compId: string) => compId !== id)
  ...
  const updateLayout = useCallback((newLayout: any[]) => {
    setLayoutData((prev: LayoutData) => {
      const updatedComponents = prev.components.map((comp: ComponentConfig) => {
  ...
  const getSelectedComponents = useCallback(() => {
    return layoutData.components.filter((comp: ComponentConfig) =>
  ...
  const getSelectedComponent = useCallback(() => {
    ...
    return layoutData.components.find((comp: ComponentConfig) =>
  ...
  const copyComponent = useCallback((id: string) => {
    const component = layoutData.components.find((comp: ComponentConfig) => comp.id === id);
    if (component) {
      setEditorState((prev: EditorState) => ({
        ...prev,
